<?php
if (!defined("_GNUBOARD_")) exit; 
global $hero_config;

// Platform Synchronized Options
$effect = isset($hero_config['effect']) ? $hero_config['effect'] : 'zoom';
$overlay_opacity = isset($hero_config['overlay']) ? $hero_config['overlay'] : 0.5;
$height = isset($hero_config['height']) ? $hero_config['height'] : '100vh';

// CSS is added in display_main_visual, but including here for safety if called directly
add_stylesheet('<link rel="stylesheet" href="'.G5_PLUGIN_URL.'/main_image_manager/skins/ultimate_hero/style.css">', 10);
?>

<div class="mi_ultimate_hero effect-<?php echo $effect; ?>" style="--hero-height: <?php echo $height; ?>;">
    <div class="swiper mainSwiper">
        <div class="swiper-wrapper">
            <?php
            // $slides is already available from display_main_visual scope
            foreach ($slides as $slide) {
                // Use the pre-calculated img_url from get_main_slides
                $img_src = $slide['img_url'];
            ?>
                <div class="swiper-slide hero_slide">
                    <div class="hero_bg_wrap">
                        <div class="hero_bg" style="background-image: url('<?php echo $img_src; ?>');"></div>
                    </div>
                    <div class="video_overlay" style="background: rgba(0,0,0,<?php echo $overlay_opacity; ?>);"></div>
                    <div class="hero_content">
                        <?php if($slide['mi_title']) { ?>
                            <h2><?php echo stripslashes($slide['mi_title']); ?></h2>
                        <?php } ?>
                        <?php if($slide['mi_desc']) { ?>
                            <p><?php echo nl2br(stripslashes($slide['mi_desc'])); ?></p>
                        <?php } ?>
                        <?php if($slide['mi_link']) { ?>
                            <a href="<?php echo $slide['mi_link']; ?>" class="btn_hero">VIEW DETAILS</a>
                        <?php } ?>
                    </div>
                </div>
            <?php } ?>
        </div>
        <div class="swiper-pagination"></div>
        <div class="scroll_down">
            <i class="fa fa-chevron-down"></i>
        </div>
    </div>
</div>

<script>
$(function() {
    if (typeof Swiper !== 'undefined') {
        new Swiper(".mainSwiper", {
            loop: true,
            effect: "fade",
            fadeEffect: { crossFade: true },
            speed: 2000,
            autoplay: { delay: 6000, disableOnInteraction: false },
            pagination: { el: ".swiper-pagination", clickable: true }
        });
    }
});
</script>
