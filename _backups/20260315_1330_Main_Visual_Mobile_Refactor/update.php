...
if ($w == "u" && isset($_POST["mi_item_id"]) && is_array($_POST["mi_item_id"])) {
    ...
    foreach ($_POST["mi_item_id"] as $item_id => $val) {
        ...
        $tag = sql_real_escape_string($_POST["mi_tag"][$item_id]);
        $title = sql_real_escape_string($_POST["mi_title"][$item_id]);
        $subtitle = sql_real_escape_string($_POST["mi_subtitle"][$item_id]);
        $desc = sql_real_escape_string($_POST["mi_desc"][$item_id]);
        $btn_text = sql_real_escape_string($_POST["mi_btn_text"][$item_id]);
        $link = sql_real_escape_string($_POST["mi_link"][$item_id]);
        $target = isset($_POST["mi_target"][$item_id]) ? clean_xss_tags($_POST["mi_target"][$item_id]) : '';
        $image_url_input = trim($_POST["mi_image_url"][$item_id]);

        $sql_data = " mi_sort = '{$sort}',
mi_tag = '{$tag}',
mi_title = '{$title}',
mi_subtitle = '{$subtitle}',
mi_desc = '{$desc}',
mi_btn_text = '{$btn_text}',
mi_target = '{$target}',
mi_link = '{$link}' ";
