...
$tm_id_target = ($w == 'u' && $tm_id_org) ? $tm_id_org : $tm_id;
...
if ($w == '') {
    // ID Check
    $check = sql_fetch(" SELECT count(*) as cnt FROM g5_plugin_top_menu_config WHERE tm_id = '{$tm_id_target}' ");
    if ($check['cnt'])
        alert('이미 존재하는 식별코드(ID)입니다.');

    $sql = " INSERT INTO g5_plugin_top_menu_config
                    SET tm_id = '{$tm_id_target}',
                        tm_skin = '{$tm_skin}',
