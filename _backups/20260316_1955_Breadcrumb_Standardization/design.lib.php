...
/**
 * Get Sub Design Group Data
 */
function get_sub_design_group($sd_id) {
    if (!$sd_id) return array();
    return sql_fetch(" SELECT * FROM " . G5_PLUGIN_SUB_DESIGN_GROUP_TABLE . " WHERE sd_id = '{$sd_id}' ");
}
...
function get_sub_design($sd_id = null, $me_code = null)
{
    ...
    if ($sd_id === null) {
        $sd_id = (isset($config["cf_theme"]) && $config["cf_theme"]) ? $config["cf_theme"] : "kukdong_panel";
    ...
    $group = get_sub_design_group($sd_id);
...
/**
 * Normalize Sub Design Image URL
 */
