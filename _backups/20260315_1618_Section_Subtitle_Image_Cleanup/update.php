...
if (isset($_POST['mc_id']) && is_array($_POST['mc_id'])) {
    foreach ($_POST['mc_id'] as $i => $mi_id) {
        ...
        $mc_tag = sql_real_escape_string($_POST['mc_tag'][$mi_id]);
        $mc_subtitle = sql_real_escape_string($_POST['mc_subtitle'][$mi_id]);
        $mc_link_text = sql_real_escape_string($_POST['mc_link_text'][$mi_id]);
        $mc_sort = $i + 1;
