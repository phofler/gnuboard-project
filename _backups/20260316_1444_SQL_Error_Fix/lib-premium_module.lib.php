...
function render_premium_id_ui($options, $legacy_id = '', $legacy_readonly = false) {
    ...
    // Theme Select
    $html .= '<select id="'.$id_field.'_theme" class="frm_input" onchange="PremiumModule.generateId(\''.$id_field.'\', \''.$onchange.'\')" '.($readonly ? 'disabled' : '').'>';
    ...
    // Lang Select
    $html .= '<select id="'.$id_field.'_lang" class="frm_input" onchange="PremiumModule.generateId(\''.$id_field.'\', \''.$onchange.'\')" '.($readonly ? 'disabled' : '').'>';
    ...
    // Custom Input
    $html .= '<input type="text" id="'.$id_field.'_custom" value="'.htmlspecialchars($parts['custom']).'" class="frm_input" placeholder="커스텀 이름" onkeyup="PremiumModule.generateId(\''.$id_field.'\', \''.$onchange.'\')" '.($readonly ? 'readonly' : '').'>';
...
function expand_premium_placeholder($content, $bridge_id = '') {
    ...
    if (strpos($content, '{MAP_API_DISPLAY}') !== false) {
        ...
        if (file_exists(G5_PLUGIN_PATH . '/map_api/lib/map.lib.php')) {
            ...
            if (function_exists('display_map_api')) {
                ob_start();
                display_map_api('100%', '600px', $bridge_id);
    ...
    // We assume these are available in $config or can be fetched via copyright_manager
    $company_data = array();
    if (file_exists(G5_PLUGIN_PATH . '/copyright_manager/lib.php')) {
        ...
        if (function_exists('get_footer_config')) {
            $company_data = get_footer_config();
        }
    }

    $replacements = array(
        '{CP_COMPANY}' => isset($company_data['cp_company']) ? $company_data['cp_company'] : $config['cf_title'],
        '{CP_CEO}'     => isset($company_data['cp_ceo']) ? $company_data['cp_ceo'] : '',
        '{CP_BIZNO}'   => isset($company_data['cp_bizno']) ? $company_data['cp_bizno'] : '',
        '{CP_TEL}'     => isset($company_data['cp_tel']) ? $company_data['cp_tel'] : '',
        '{CP_FAX}'     => isset($company_data['cp_fax']) ? $company_data['cp_fax'] : '',
        '{CP_ADDRESS}' => isset($company_data['cp_address']) ? $company_data['cp_address'] : '',
        '{CP_EMAIL}'   => isset($company_data['cp_email']) ? $company_data['cp_email'] : $config['cf_admin_email'],
        '{COMPANY}'    => isset($company_data['cp_company']) ? $company_data['cp_company'] : $config['cf_title'],
        '{ADDR}'       => isset($company_data['cp_address']) ? $company_data['cp_address'] : '',
    );
