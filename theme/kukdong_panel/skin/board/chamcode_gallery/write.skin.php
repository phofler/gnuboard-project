    ...
    <form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);"
        ...
        <?php if ($is_category) { ?>
            <div class="bo_w_select write_div">
                ...
                <div id="category_container" class="category-wrapper">
            ...
            <script>
                ...
                function initCascadingCategory(boTable) {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            ...
                            renderCascadingSelects(treeData, $wrap, $originalSelect, $codeStorage, initialCaName, initialCode);
                        },
                        error: function () {
                            $originalSelect.show();
                        }
                ...
                function renderCascadingSelects(treeData, $container, $originalSelect, $codeStorage, initialCaName, initialCode) {
                    ...
                    $depth2.on("change", function () {
                        ...
                        if (val) {
                            ...
                            var parentNode = findNode(treeData, parentVal); // Optimize?
                            if (parentNode) {
                    ...
                    function updateFinalValues() {
                        ...
                        var fullPath = "";
                        if (d1) fullPath += $depth1.find("option:selected").text();
                        if (d2) fullPath += " > " + $depth2.find("option:selected").text();
                        if (d3) fullPath += " > " + $depth3.find("option:selected").text();

                        $originalSelect.val(fullPath);
                    }

                    // Initial Load Logic (Priority: URL Param > wr_1 > ca_name parsing)
                    var urlParams = new URLSearchParams(window.location.search);
                    // me_code or cate or initialCode
                    var targetCode = urlParams.get('me_code') || urlParams.get('cate') || initialCode;

                    if (targetCode) {
                        // Smart Select by Code
                        // Reverse lookup or just flatten to find path?
                        // Simplest: Iterate segments 2-digits at a time
                        ...
                        if (seg1.length === 2 && $depth1.find("option[value='" + seg1 + "']").length) {
                            ...
                            if (targetCode.length >= 4) {
                                var seg2 = targetCode.substring(0, 4); // Assuming standard is 1010 not 10-10? Wait.
                                // Admin logic said "1010". Yes.
                                // But my treeData nodes use FULL codes.
                                // Yes: Depth1=10, Depth2=1010, Depth3=101010.
                        ...
                        // Fallback to text matching if no code
                        var parts = initialCaName.split(" > ");
                        // ... (implement text matching if needed, but code is better)
                    }
                }
            </script>
        <?php } ?>
