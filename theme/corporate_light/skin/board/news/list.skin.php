...
<div class="sub-layout-width-height">
    <div id="bo_list">
        ...
        <!-- Theme Standardization: News Header (Left Aligned to match Company Intro) -->
        <div class="section-header" data-aos="fade-right">
            <p style="color:var(--color-accent-gold); font-weight:bold; letter-spacing:1px; margin-bottom:10px;">News &
                Notice</p>
            <h2 style="font-size:3rem; margin-bottom:0; text-transform:uppercase;"><?php echo $board['bo_subject'] ?>
            </h2>
            <p style="color:var(--color-text-secondary); margin-top:20px; font-weight:normal;">르네상스의 새로운 소식을 전해드립니다.</p>
        </div>
                ...
                <table>
                    ...
                    <thead>
                        <tr>
                            ...
                            <?php if ($is_nogood) { ?>
                                <th scope="col"><?php echo subject_sort_link('wr_nogood', $qstr2, 1) ?>비추천 </a></th>
                            <?php } ?>
            ...
            <fieldset class="bo_sch">
                ...
                <form name="fsearch" method="get">
                    ...
                    <button type="button" class="bo_sch_cls" title="닫기"><i class="fa fa-times"
                            aria-hidden="true"></i><span class="sound_only">닫기</span></button>
                </form>
            </fieldset>
    ...
    <?php if ($is_checkbox) { ?>
        <script>         function all_checked(sw) {             var f = document.fboardlist;
                 for (var i = 0; i < f.length; i++) {                 if (f.elements[i].name == "chk_wr_id[]")                     f.elements[i].checked = sw;             }         }
             function fboardlist_submit(f) {             var chk_count = 0;
                 ...
                 return true;         }
             // 선택한 게시물 복사 및 이동         function select_copy(sw) {             var f = document.fboardlist;
                 if (sw == "copy")                 str = "복사";             else                 str = "이동";
                 var sub_win = window.open("", "move", "left=50, top=50, width=500, height=550, scrollbars=1");
                 f.sw.value = sw;             f.target = "move";             f.action = g5_bbs_url + "/move.php";             f.submit();         }
             // 게시판 리스트 관리자 옵션         jQuery(function ($) {             $(".btn_more_opt.is_list_btn").on("click", function (e) {                 e.stopPropagation();                 $(".more_opt.is_list_btn").toggle();             });             $(document).on("click", function (e) {                 if (!$(e.target).closest('.is_list_btn').length) {                     $(".more_opt.is_list_btn").hide();                 }             });         });
        </script>
    <?php } ?>
</div>
<!-- } 게시판 목록 끝 -->