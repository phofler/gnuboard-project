...
<article id="bo_v" class="skin-container" style="width:<?php echo $width; ?>">
    <?php
    // [UI FIX] Preserve me_code in all navigation links
    if (isset($_GET['me_code'])) {
        $me_param = '&me_code=' . urlencode($_GET['me_code']);
        if ($list_href)
            $list_href .= $me_param;
        if ($reply_href)
            $reply_href .= $me_param;
        if ($write_href)
            $write_href .= $me_param;
        if ($update_href)
            $update_href .= $me_param;
        if ($delete_href)
            $delete_href .= $me_param;
        // Search href might already have localized query, but usually safe to append
        if ($search_href)
            $search_href .= $me_param;
    }
    ?>
    <header>
        <h2 id="bo_v_title">
