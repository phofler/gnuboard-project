...
if (!function_exists('get_latest_skin_config')) {
    function get_latest_skin_config(string $ls_id)
    {
        ...
        // [Premium Standardization] Global Skin Context 설정
        if (function_exists('set_pro_skin_context')) {
            $board = get_board_db($ls['ls_bo_table'], true);
            $context_data = array(
                'title'     => ($ls['ls_title']) ? $ls['ls_title'] : get_text($board['bo_subject']),
                'desc'      => $ls['ls_description'],
                'more_url'  => $ls['ls_more_link'],
                'skin_cls'  => 'sec-' . str_replace('plugin/', '', $ls['ls_skin']),
                'options'   => $ls
            );
            // items(list)는 추출 함수 내부에서 처리되므로 여기서는 구조만 전달
            set_pro_skin_context($context_data);
        }
...
if (!function_exists('latest_plugin_skin')) {
    function latest_plugin_skin($skin_dir, $bo_table, $rows=10, $subject_len=40, $cache_time=1, $options='')
    {
        ...
        // [Premium Standardization] List 데이터 전역화 재수행 (list 변수 확실히 전달)
        if (function_exists('set_pro_skin_context')) {
            set_pro_skin_context(array('items' => $list));
        }
