                        ...
                        <tr class="bg<?php echo $i % 2; ?>">
                            ...
                            <td class="td_left" style="padding:10px;">
                                <div style="display:flex;">
                                    <div id="preview_wrap_<?php echo $item_id; ?>" style="width:200px; margin-right:20px;">
                                        <?php if ($img_src) { ?>
                                            <img src="<?php echo $img_src; ?>" id="preview_<?php echo $item_id; ?>"
                                                style="width:100%; height:120px; border:1px solid #ddd; object-fit:cover; display:block;">
                                        <?php } else { ?>
                                            <div class="mi_preview_box" id="preview_<?php echo $item_id; ?>"
                                                ...
                                                NO IMAGE<br><?php echo ($mi_group_config["mi_skin"] == "basic") ? "640x960" : "1920x1080"; ?>
                                            </div>
                                        <?php } ?>
                                        ...
                                        <div style="display:flex; gap:5px; margin-top:10px;">
                                            <button type="button" class="btn btn_03" style="flex:1; padding:7px 0; font-size:11px;"
                                                onclick="openImageManager('<?php echo $item_id; ?>')">이미지 변경</button>
                                            <button type="button" class="btn btn_02" style="flex:1; padding:7px 0; font-size:11px; background:#f1f1f1; border-color:#ccc; color:#666;"
                                                onclick="clear_mi_image('<?php echo $item_id; ?>')">이미지 삭제</button>
                                        </div>
                                    </div>
                                    ...
                                    <div style="flex:1;">
                                        ...
                                        <textarea name="mi_desc[<?php echo $item_id; ?>]" class="frm_input full_input"
                                            style="margin-top:5px; height:60px;"
                                        ...
                                        <div style="display:flex; gap:5px; margin-top:5px; align-items:center;">
                                            ...
                                            <label style="white-space:nowrap; margin-left:5px;">
                                                <input type="checkbox" name="mi_target[<?php echo $item_id; ?>]" value="_blank" <?php echo ($row["mi_target"] == "_blank") ? "checked" : ""; ?>> 새 창
                                            </label>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="td_mng td_center">
                                <button type="button" class="btn_02 btn_del_ajax" data-id="<?php echo $item_id; ?>">슬라이드 삭제</button>
                            </td>
                        </tr>
                        <?php $i++;
                    } ?>
...
#unsplash_modal_content {
    width: 98%; max-width: 1800px; height: 96%; max-height: 1200px; background: #fff; position: relative; border-radius: 8px; overflow: hidden; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3); margin: 1% auto;
}
#unsplash_iframe {
    width: 100%; height: 100%; border: none;
}
.close-modal {
    position: absolute; top: 15px; right: 20px; font-size: 24px; color: #333; cursor: pointer; z-index: 10000; width: 35px; height: 35px; line-height: 35px; text-align: center; background: rgba(255, 255, 255, 0.9); border-radius: 50%; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); transition: all 0.2s;
}
</style>
...
<script>
        ...
        if (mi_id) {
            ...
            if ($preview.length) {
                if ($preview.is("img")) {
                    ...
                    $preview.replaceWith("<img id=\"preview_" + mi_id + "\" src=\"" + url + "\" style=\"width:100%; height:120px; border:1px solid #ddd; object-fit:cover; display:block;\">");
                }
            }
            closeUnsplashModal();
        }
    ...
    function clear_mi_image(id) {
        ...
        if (is_new) {
            ...
            $.post('<?php echo G5_ADMIN_URL; ?>/ajax.unified_image_delete.php', {
                type: 'main_image',
                id: id,
                token: $('#fmainimage input[name="token"]').val()
            }, function(res) {
                if (res.success) {
                    reset_mi_ui(id);
                } else {
    ...
    function reset_mi_ui(id) {
        $('#mi_image_url_' + id).val('');
        var skin = $("input[name='mi_skin']:checked").val();
        var size = (skin == "basic") ? "640x960" : "1920x1080";
        var placeholder = "<div id=\"preview_" + id + "\" style=\"width:100%; height:120px; background:#f5f5f5; border:1px solid #ddd; display:flex; align-items:center; justify-content:center; color:#999; font-size:11px; text-align:center;\">NO IMAGE<br>" + size + "</div>";
        $('#preview_' + id).replaceWith(placeholder);
    }

    $(function () {
        $(document).on("click", ".btn_del_ajax", function () {
            if (!confirm("슬라이드 전체를 삭제하시겠습니까?")) return;
