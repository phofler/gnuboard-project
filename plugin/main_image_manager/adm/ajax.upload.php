...
// [UNIFIED] Theme-specific Directory Setup (Same as Main Content Manager)
$theme_name = isset($config['cf_theme']) ? $config['cf_theme'] : 'default';
$base_dir = G5_DATA_PATH . '/common_assets';
$upload_dir = $base_dir . '/' . $theme_name . '/';
$upload_url = G5_DATA_URL . '/common_assets/' . $theme_name . '/';

if (!is_dir($base_dir)) {
    @mkdir($base_dir, G5_DIR_PERMISSION, true);
    @chmod($base_dir, G5_DIR_PERMISSION);
}
...
if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
    ...
    if (!in_array($file_ext, $allowed_ext)) {
        die(json_encode(array('error' => '허용되지 않는 파일 형식입니다.')));
    }
