...
<!-- Modern Premium Dark Skin (Restored + Multi-Column Support) -->
<div class="modern-gnb-wrap">
        ...
        <ul id="gnb_1dul">
            <?php foreach ($menu_datas as $row) {
                ...
                <li class="gnb_1dli">
                    ...
                    <?php if ($has_sub) { ?>
                        <!-- Refactored: Use CSS Columns instead of multiple ULs for reliability -->
                        <ul class="gnb_2dul <?php echo $is_wide ? 'wide-dropdown' : ''; ?>"
                            style="<?php echo $is_wide ? 'display:block !important; column-count:2; column-gap:30px; min-width:400px;' : ''; ?>">
                            <?php
    ...
    <button type="button" class="gnb_menu_btn" id="gnb_menu_btn"
        style="display:none; font-size:24px; background:none; border:none; color:#fff; cursor:pointer; margin-left:15px;">
    ...
    <div class="hd_right" style="display: flex; align-items: center; gap: 20px;">
        ...
        <a href="mailto:<?php echo $config['cf_admin_email']; ?>" class="btn-inquiry">
            Contact Us
        </a>
    </div>
...
<!-- Full Screen Mobile Overlay Menu (Imported for Modern Dark) -->
<div id="gnb_all">
...
<script>
    $(function () {
        // [IMPORTANT] Move the mobile menu to the body tag to avoid Z-index issues
