...
<div style="display:flex; gap:20px;">
                ...
                <tr>
                    ...
                    <td>
                        <style>
                            .skin-group { border: 1px solid #eee; border-radius: 8px; padding: 20px; background: #fff; }
                            .skin-list { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; }
                            .skin-item { cursor: pointer; transition: all 0.2s; }
                            .skin-item label { display: block; padding: 15px; border: 1px solid #ddd; border-radius: 6px; background: #fafafa; cursor: pointer; text-align: center; transition: all 0.2s; }
                            .skin-item label:hover { border-color: #999; background: #f0f0f0; }
                            .skin-item input[type="radio"] { display: none; }
                            .skin-item input[type="radio"]:checked+label { border-color: #3498db; background: #ebf5fb; box-shadow: 0 4px 8px rgba(52, 152, 219, 0.1); }
                            .skin-name { display: block; font-weight: bold; font-size: 14px; margin-bottom: 5px; color: #333; }
                            .skin-desc { display: block; font-size: 12px; color: #777; }
                        </style>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div style="display:flex; align-items:center; gap:5px;">
                            <input type="text" name="cp_bgcolor" value="<?php echo $current_bg; ?>" id="cp_bgcolor" class="frm_input" size="10">
                            <input type="color" id="cp_bgcolor_picker" value="<?php echo $current_bg; ?>" onchange="document.getElementById('cp_bgcolor').value = this.value; update_editor_background(this.value);">
                            <button type="button" class="btn_frmline" style="height:24px; line-height:24px; padding:0 10px;" onclick="restore_default_bgcolor('<?php echo $default_theme_bg; ?>');">기본값 복원</button>
                        </div>
                    </td>
                </tr>
            </tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="frm_info" style="margin-bottom:10px; color:#2980b9;">
                            ※ 위 하단 정보(Quick Info)에 입력한 내용은 <strong>{addr}, {tel}, {fax}, {email}, {logo}, {copyright}, {link1}, {link2}</strong> 치환자로 자동 치환되어 에디터에 로드됩니다.
                        </div>
                        <?php
...
<script>
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (typeof oEditors !== 'undefined' && oEditors.getById["cp_content"]) {
                    oEditors.getById["cp_content"].exec("SET_IR", [content]); 
                    setTimeout(function () { update_editor_background($('#cp_bgcolor').val()); }, 300);
                } else {
    ...
    function restore_default_bgcolor(defaultColor) {
        ...
        $('#cp_bgcolor_picker').val(defaultColor);
        update_editor_background(defaultColor);
    }

    $(function () {
        setTimeout(function () { update_editor_background($('#cp_bgcolor').val()); }, 500);
        $('#cp_bgcolor').on('change keyup', function () { update_editor_background($(this).val()); });
    });
</script>
