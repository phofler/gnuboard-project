...
function get_sub_design($sd_id = '', $me_code = '') {
    ...
    // 1. 서브 디자인 그룹 찾기
    $sql = " SELECT * FROM " . G5_PLUGIN_SUB_DESIGN_GROUP_TABLE . " WHERE sd_id = '{$sd_id}' ";
    $sd = sql_fetch($sql);
    ...
    // 3. 해당 메뉴의 세부 설정 가져오기
    $sql_item = " SELECT * FROM " . G5_PLUGIN_SUB_DESIGN_ITEM_TABLE . " 
    ...
    if (!$item && strlen($me_code) > 2) {
        ...
        $sql_parent = " SELECT * FROM " . G5_PLUGIN_SUB_DESIGN_ITEM_TABLE . " 
...
function display_sub_visual($sd = array()) {
    ...
    // Load standardized typography CSS
    echo '<link rel="stylesheet" href="' . G5_PLUGIN_URL . '/sub_design/skins/typography.css?ver=' . G5_TIME_YMDHIS . '">';
    ...
    if (file_exists($skin_path)) {
        ...
        ?>
        <section class="sub-hero sub-hero-fallback">
            <div class="sub-hero-bg" style="background-image: url('<?php echo $sd_img_url; ?>');"></div>
            <div class="sub-hero-content">
                <h1 class="sub-hero-title"><?php echo $item['sd_main_text']; ?></h1>
                <?php if (isset($item['sd_sub_text']) && $item['sd_sub_text']) { ?>
                    <p class="sub-hero-subtitle"><?php echo $item['sd_sub_text']; ?></p>
                <?php } ?>
        ...
        <style>
        .sub-hero-fallback { position: relative; height: 400px; display: flex; align-items: center; justify-content: center; overflow: hidden; background: #000; text-align: center; }
        .sub-hero-fallback .sub-hero-bg { position: absolute; inset: 0; background-size: cover; background-position: center; filter: brightness(0.5); }
        .sub-hero-fallback .sub-hero-content { position: relative; z-index: 2; padding: 20px; }
        </style>
        <?php
    }
}
